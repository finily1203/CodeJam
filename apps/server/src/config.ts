...
const envSchema = z.object({
  ...
  ARK_MODEL_CONTEXT_WINDOW: z.coerce.number().int().positive().default(128_000),
...
export function loadConfig(environment: NodeJS.ProcessEnv = process.env) {
  ...
  return {
    ...
    arkModelContextWindow: env.ARK_MODEL_CONTEXT_WINDOW,
...
const MODEL_CATALOG_FILENAME = "model-catalog.json";

/**
 * Codex resolves the configured model against a catalog embedded in its own
 * binary, which only covers OpenAI's models. A third-party model such as the
 * Ark one is not in it, so Codex emits
 *   "Model metadata for `<model>` not found. Defaulting to fallback metadata"
 * through the same `item.type: "error"` event it uses for genuine failures —
 * and the trace parser records that as a failed span on every single run.
 *
 * Supplying our own catalog entry makes the lookup succeed, so the warning is
 * never emitted. Every field here is required by Codex 0.111's parser.
 */
function buildModelCatalog(config: AppConfig): string {
  const model = config.arkModel || "ep-not-configured";
      ...
      models: [
        {
          slug: model,
          display_name: model,
          description: "Model served through the Ark provider.",
          context_window: config.arkModelContextWindow,
          ...
          supported_reasoning_levels: [
            { effort: "medium", description: "General-purpose reasoning" },
          ],
          // Required by the parser, but an empty string is accepted and Codex
          // keeps using its own default instructions — verified with a live run
          // in which the agent still enforced its AGENTS.md rules.
          base_instructions: "",
...
export async function writeCodexConfig(config: AppConfig): Promise<void> {
  ...
  await writeFile(path.join(config.codexHome, MODEL_CATALOG_FILENAME), buildModelCatalog(config), {
    encoding: "utf8",
    mode: 0o600,
  });
  const toml = [
    ...
    // Relative to CODEX_HOME, so it resolves both on the host and in the
    // container, which mounts codexHome at a different absolute path.
    "model_catalog_json = " + JSON.stringify(MODEL_CATALOG_FILENAME),
    "",
