...
const updateAgentBody = createAgentBody
  .partial()
  .extend({ budgetLimitUsd: z.number().min(0).nullable().optional() })
...
export async function createApp(
  ...
  app.get("/api/runs/:id/trace", async (request) => {
    const { id } = runIdParams.parse(request.params);
    return service.getRunTrace(id);
  });

  app.post("/api/runs/:id/explain", async (request) => {
    const { id } = runIdParams.parse(request.params);
    return { run: await service.explainRun(id) };
  });
